...
- **Restarts.** A run still `running` when the server stopped becomes `interrupted` at the next start. Its effects may be partial, so it never runs again by itself. `jobs.resolve(id, 'retry')` starts a new run with the same input and `key`, and `'dismiss'` leaves it as is.
