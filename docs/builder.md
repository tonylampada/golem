...
For implementation and pull-request work, start with the user or business problem and the resulting behavior. Keep a PR description to one short paragraph; add terse validation and dependencies only when useful. Run focused checks appropriate to the change. Run the no-mistakes pipeline only when the person explicitly asks for it.
