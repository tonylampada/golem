...
import { knowledge, openView, type ViewOffer } from '../client'
import { setChatView } from './adapters'

export type OpenSource = ViewOffer['input'] & { key: string }

/**
 * This tab's view of one chat: the assistant's offers to open a source, answered here. Only an
...
export function useSourceView(conversation: string | undefined, onOpen: (source: OpenSource) => void) {
  ...
  const view = useRef<ReturnType<typeof openView>>(undefined)
  const open = useRef(onOpen)
  open.current = onOpen
  useEffect(() => {
    setOffers([])
    if (!conversation) return
    const current = openView(conversation, (event) => {
      if (event.type === 'offer') setOffers((list) => [...list.filter((one) => one.id !== event.offer.id), event.offer])
    ...
    current.id.then((id) => setChatView(id), () => setChatView(undefined))
    return () => { current.close(); setChatView(undefined) }
  }, [conversation])
  const answer = async (offer: ViewOffer, accept: boolean) => {
    setError(undefined)
    setOffers((list) => list.filter((one) => one.id !== offer.id))
    try { await view.current?.answer(offer.id, accept) } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) }
  }
  const panel = (offers.length > 0 || error) && (
    <div className="golem-browser-offers border-t border-neutral-200 px-4 py-2 text-sm">
      {offers.map((offer) => (
        <div key={offer.id} className="golem-browser-offer flex flex-wrap items-center gap-2 py-1">
          <span className="min-w-0 flex-1 truncate">Open {offer.input.path}, lines {offer.input.line}–{offer.input.endLine}?</span>
          <button className="rounded bg-neutral-900 px-2 py-1 text-white" onClick={() => void answer(offer, true)}>Open</button>
          <button className="rounded border border-neutral-300 px-2 py-1" onClick={() => void answer(offer, false)}>Dismiss</button>
        </div>
      ))}
...
export function SourcePanel({ source, onClose }: { source: OpenSource; onClose(): void }) {
  return (
    <div className="golem-browser-source flex h-full flex-col overflow-hidden">
      <div className="flex items-center justify-between gap-2 border-b border-neutral-200 px-4 py-2 text-sm">
        <span className="truncate font-medium">{source.path}</span>
        <button type="button" className="rounded border border-neutral-300 px-2 py-1" onClick={onClose}>Back to app</button>
      </div>
