...
- **The mark follows the text on screen.** The Editor may still show an older version of the file, or the person's unsaved edits. The passage is marked only after the Editor has the version it was found in, or a later one, and has no conflict open. It is marked at the offered lines when the Editor shows exactly that version. Otherwise it is marked where the passage's exact text now is. When the text is missing, or appears more than once, nothing is marked and the header says why. Edits are never changed to make room for a mark.
